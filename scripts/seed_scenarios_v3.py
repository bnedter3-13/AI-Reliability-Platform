"""
scripts/seed_scenarios_v3.py

Same insert-only pattern as scripts/seed_scenarios_v2.py, loading
data/seed_scenarios_v3.json instead. This batch reflects the further
GENERATION_SYSTEM_PROMPT strengthening (app/comparison/model_comparator.py) -
more forceful MUST/NOT phrasing, an explicit refusal example, and an explicit
block on the "helpful guess" failure mode. GPT-4o now fully complies with
grounding instructions; Qwen Plus still fabricates unsupported specifics when
context is present but insufficient/thin (poor_retrieval, hallucination).

`evaluator_version` is pulled live from EVALUATOR_VERSION in
app.evaluation.evaluator (now "claude-sonnet-5:gen1.1.0:judge1.1.0" following
the new GENERATION_PROMPT_VERSION constant in app/evaluation/prompts.py,
introduced specifically so this batch is distinguishable from
data/seed_scenarios_v2.json's "claude-sonnet-5:1.1.0" - that tag pre-dates the
gen/judge split and doesn't separately reflect this generation-prompt change).

Timestamps: same anchoring approach as v2 - anchors ANCHOR_OFFSET_HOURS after
the current max created_at in the evaluations table (which now includes batch
2's rows), and spreads forward either to the real current time or across a
fixed fallback window - see _compute_time_range().

Usage:
    python scripts/seed_scenarios_v3.py
"""
...
SCENARIOS_PATH = Path(__file__).resolve().parent.parent / "data" / "seed_scenarios_v3.json"
