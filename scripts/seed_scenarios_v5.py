"""
scripts/seed_scenarios_v5.py

Same insert-only pattern as scripts/seed_scenarios_v4.py, loading
data/seed_scenarios_v5.json instead. This batch reflects two fixes verified live
together: (1) evaluator.py no longer assumes message.content[0] is the judge's
text block - it now scans for the first block with type=="text", so a leading
ThinkingBlock from claude-sonnet-5's extended thinking no longer degrades
evaluation to status="error"; (2) compare_models() now makes a second, more
pointed retry (RETRY_CORRECTIVE_MESSAGE_2) when the first retry still fails,
tracked by COMPARATOR_VERSION "1.1.0" in app/evaluation/prompts.py.

`evaluator_version` is pulled live from EVALUATOR_VERSION in app.evaluation.evaluator
(now "claude-sonnet-5:gen1.1.0:judge1.1.0:cmp1.1.0" following the COMPARATOR_VERSION
bump) - distinct from data/seed_scenarios_v4.json's "...:cmp1.0.0".

Timestamps: same anchoring approach as v2/v3/v4 - anchors ANCHOR_OFFSET_HOURS after
the current max created_at in the evaluations table (which now includes batch 4's
rows), and spreads forward either to the real current time or across a fixed
fallback window - see _compute_time_range().

Usage:
    python scripts/seed_scenarios_v5.py
"""
...
SCENARIOS_PATH = Path(__file__).resolve().parent.parent / "data" / "seed_scenarios_v5.json"
